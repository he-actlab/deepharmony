...
from deepharmony.tf_utils.dataset import ImageNetProducer
from deepharmony.tf_utils.helper import DataSpec
...
def get_tf_performance(deepharmony_graph, phase):
    train = phase == 'forward+backward'
    print(train, phase)
    g, train_op, sparsity_op, data, labels, logits = create_tf_graph('random', deepharmony_graph, train)
    pmon = GPUPowerMonitor(0)

    input_shape = deepharmony_graph.tensor_registry['inputs/data'].shape
    label_shape = deepharmony_graph.tensor_registry['inputs/labels'].shape
...
def get_tf_breakdown(deepharmony_graph):
    g, train_op, sparsity_op, data, labels, logits = create_tf_graph('random', deepharmony_graph, train=True)

    input_shape = deepharmony_graph.tensor_registry['inputs/data'].shape
    label_shape = deepharmony_graph.tensor_registry['inputs/labels'].shape

    log_path = '/home/hardik/workspace/tf-logs/breakdown/{}/summary'.format(deepharmony_graph.name)
    save_path = '/home/hardik/workspace/tf-logs/breakdown/{}/ckpt'.format(deepharmony_graph.name)
...
def get_tf_sparsity(deepharmony_graph, num_epochs=10):
    print(deepharmony_graph.name)
    g, train_op, sparsity_ops, data, labels, logits = create_tf_graph('alexnet', deepharmony_graph, train=True)

    data_shape = deepharmony_graph.tensor_registry['inputs/data'].shape
    ...
    with g.as_default():
        ...
        log_path = '/home/hardik/workspace/tf-logs/train/{}/summary'.format(deepharmony_graph.name)
        save_path = '/home/hardik/workspace/tf-logs/train/{}/ckpt'.format(deepharmony_graph.name)
        save_file = os.path.join(save_path, 'model.ckpt')
