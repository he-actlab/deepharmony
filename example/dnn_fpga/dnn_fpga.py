...
import numpy as np
import deepharmony.benchmarks
import deepharmony.benchmarks.yolo2_tiny
import deepharmony.compiler
import logging 
from deepharmony.fpga.fpgamanager import FPGAManager
import deepharmony.simulator.accelerator

def initialize_yolo_graph(weight_pickle, debug_mode=False):
    yolo_graph = deepharmony.benchmarks.get_graph('yolo2_tiny', train=False)

    fpga_spec = deepharmony.compiler.FPGASpec(num_ddr=1, size_ddr=1024, bandwidth_per_ddr=512)
    fpga_compiler = deepharmony.compiler.GraphCompiler(fpga_spec)
    ...
    acc_obj = deepharmony.simulator.accelerator.Accelerator(N=32, M=32, prec=16, mem_if_width=256, frequency=100e6, sram=sram)
